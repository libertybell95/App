...
Customer.bulkCreate = (req_arr,result) =>{
  pg.query("INSERT INTO customers(id, level_col, cvss, title, vulnerability, solution, reference_col) VALUES ?", [req_arr], (err, res) => {
    if (err) {
...
Customer.create = (newCustomer, result) => {
  pg.query("INSERT INTO customers SET ?", newCustomer, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Customer.findById = (customerId, result) => {
  pg.query(`SELECT * FROM customers WHERE id = ${customerId}`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
...
Customer.remove = (id, result) => {
  pg.query("DELETE FROM customers WHERE id = ?", id, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Customer.removeAll = result => {
  pg.query("DELETE FROM customers", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
